...
HTTPRequest KNExecutionEngine::generate_request( const Thunk & thunk )
{
...
void KNExecutionEngine::force_thunk( const Thunk & thunk,
                                            ExecutionLoop & exec_loop )
{
  HTTPRequest request = generate_request( thunk );

  uint64_t connection_id = exec_loop.make_http_request<TCPConnection>( thunk.hash(),
    address_, request,
...
size_t KNExecutionEngine::job_count() const
{
